...
function showResults() {
  ...
  const redText = getCorrectText(yourRed - correctRed, "r");
  const greenText = getCorrectText(yourGreen - correctGreen, "g");
  const blueText = getCorrectText(yourBlue - correctBlue, "b");
  let stats = document.createElement("p");
  let preview = document.getElementById("preview");
  preview.style.display = "none";
  stats.innerHTML = `correct:    #${correctColor}<br>your guess: #${guessedValue}<br><br>${redText}<br>${greenText}<br>${blueText}<br><br>go again?<br><br><button onclick="location.reload()">retry</button>`
  document.querySelector("main").append(stats)
}

function getCorrectText(factor, color) {
  //console.log(factor)
  if (factor < 1) return `your ${color} was ${factor}`
  return `your ${color} was +${factor}`
}